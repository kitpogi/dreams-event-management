...
const ManageClients = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      // For now, we'll fetch bookings and extract unique clients
      ...
      allBookings.forEach(booking => {
        if (booking.user && !clientsMap.has(booking.user.id)) {
          clientsMap.set(booking.user.id, {
            ...booking.user,
            bookings_count: 1
          });
        } else if (booking.user) {
          const client = clientsMap.get(booking.user.id);
          client.bookings_count = (client.bookings_count || 0) + 1;
        }
      });
